...
class HemeraRenderer:
    def __init__(self, viewport_h: int = 0, viewport_w: int = 0):
        ...
        self.term_h = terminal_size.lines - 30
        self.term_w = terminal_size.columns
    ...
    def render(self, buffer: np.ndarray):
        self.generate_working_frame()
    ...
    def generate_working_frame(self):
        self.working_frame = np.zeros((self.term_h, self.term_w), dtype=np.uint8)
